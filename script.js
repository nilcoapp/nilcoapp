...
/* ===== SERVER SYNC (ExtendsClass JSON Bin) ===== */
let _syncBusy = false;
...
async function pullFromServer(){
  ...
  try {
    const resp = await fetch(JSON_BIN_URL, {cache:'no-store'});
...
/* Write full state to remote (read-merge-write to avoid overwriting others' data) */
async function pushToServer(){
  if(!JSON_BIN_URL || _syncBusy) return;
  _syncBusy = true;
  try {
    // 1. Read latest remote data
    const getResp = await fetch(JSON_BIN_URL, {cache:'no-store'});
    let remote = {products:[],invoices:[],followups:[],clients:[],users:[],lastUpdated:0,invoiceCounter:0};
    if(getResp.ok) {
      const r = await getResp.json();
      if(r && typeof r === 'object') remote = r;
    }
    // 2. Merge local into remote
    // Products: local is source of truth when admin pushes, otherwise remote
    ...
    (state.data.followups || []).forEach(f => {
      if(!remoteFollowIds.has(f.id)) { allFollowups.push(f); remoteFollowIds.add(f.id); }
    });
    remote.followups = allFollowups;
    state.data.followups = allFollowups;
    // Clients: admin is source of truth
    ...
    // Users: admin is source of truth
    ...
    // Invoice counter: take the max
    remote.invoiceCounter = Math.max(remote.invoiceCounter || 0, state.data.invoiceCounter || 0);
    state.data.invoiceCounter = remote.invoiceCounter;
    // Online users: merge
    if(!remote.onlineUsers) remote.onlineUsers = {};
    if(state.data.onlineUsers) {
      Object.entries(state.data.onlineUsers).forEach(([id, u]) => {
        if(!remote.onlineUsers[id] || u.lastSeen > remote.onlineUsers[id].lastSeen) {
          remote.onlineUsers[id] = u;
        }
      });
    }
    // Messages: merge
    ...
    (state.data.messages || []).forEach(m => {
      if(!remoteMsgIds.has(m.id)) { remote.messages.push(m); remoteMsgIds.add(m.id); }
      else {
        const rm = remote.messages.find(x => x.id === m.id);
        if(rm && m.readBy) { if(!rm.readBy) rm.readBy = []; m.readBy.forEach(r => { if(!rm.readBy.includes(r)) rm.readBy.push(r); }); }
      }
    });
    remote.messages.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
    if(remote.messages.length > 100) remote.messages = remote.messages.slice(0, 100);
    state.data.messages = remote.messages;
    // Inventories: merge
    ...
    (state.data.inventories || []).forEach(inv => {
      if(!remoteInvIds.has(inv.id)) { remote.inventories.push(inv); remoteInvIds.add(inv.id); }
    });
    state.data.inventories = remote.inventories;
...
async function syncToServer(){ await pushToServer(); }
    ...
    serverData.messages.forEach(m => {
      if(!localMsgIds.has(m.id)) {
        ...
        if(local && m.readBy) {
          ...
          m.readBy.forEach(r => { if(!local.readBy.includes(r)) local.readBy.push(r); });
        }
      }
    });
...
function startSyncPolling(){
  ...
  syncInterval = setInterval(pullFromServer, 30000);
  pullFromServer();
}
